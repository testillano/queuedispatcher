...
usage() {
  ...
  Usage: $0 [--builder-image|--project|--project-image|--auto]
         ...
         --project:       make_procs, build_type, base_tag (queuedispatcher_builder)
         --project-image: image_tag, base_tag (queuedispatcher_builder), make_procs, build_type
...
build_builder_image() {
  echo
  echo "=== Build queuedispatcher_builder image ==="
  echo
  _read image_tag
  _read base_tag
  _read make_procs
  ...
  docker build --rm ${DBUILD_XTRA_OPTS} ${bargs} -f Dockerfile.build -t ${registry}/queuedispatcher_builder:"${image_tag}" . || return 1
  set +x
}
...
build_project() {
  ...
  echo
  echo "=== Build queuedispatcher project ==="
  echo
  _read base_tag
  _read make_procs
  _read build_type
  ...
  rm -f CMakeCache.txt
  # shellcheck disable=SC2086
  docker run --rm -it -u "$(id -u):$(id -g)" ${envs} -v "${PWD}":/code -w /code ${registry}/queuedispatcher_builder:"${base_tag}" || return 1
  # shellcheck disable=SC2086
  docker run --rm -it -u "$(id -u):$(id -g)" ${envs} -v "${PWD}":/code -w /code ${registry}/queuedispatcher_builder:"${base_tag}" "" doc || return 1
  set +x
}

build_project_image() {
  echo
  echo "=== Build queuedispatcher image ==="
  echo
  _read image_tag
  _read base_tag
  _read make_procs
  ...
  rm -f CMakeCache.txt
  # shellcheck disable=SC2086
  docker build --rm ${DBUILD_XTRA_OPTS} ${bargs} -t ${registry}/queuedispatcher:"${image_tag}" . || return 1
  set +x
}
